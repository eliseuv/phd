...
md"""
Select parameters:
-  $n_{series}$: number of time series per matrix
-  $t_{max}$: length of each time series
-  $n_{samples}$: total number of time series matrices
"""
...
M_samples = [rand(Normal(), t_max, n_series) for _ ∈ 1:n_samples]

# ╔═╡ 4a07dcb2-0f73-4f28-bf9b-e061fbed58f2
md"""
Calculate vector with pairwise correlations.
"""

# ╔═╡ 87da073c-c8eb-4696-ab44-faca0b2800ef
plot(x=mapreduce(ts_correlations, vcat,	M_samples),
	Geom.histogram(density=true),
	Guide.title("Initial correlation distribuiton"))

# ╔═╡ f6073803-5000-4e49-8b70-99e61c4cc6bd
md"""
Set target correlation distribution to $\rho_{dist}$.
"""

# ╔═╡ 93b2a6b1-57c8-401d-838e-abed0b0ce8dc
const ρ_dist = Uniform(-1.0, 1.0)

# ╔═╡ 9f6764c5-965c-4e26-8b6a-f96032296642
md"""
Define perturbation method.
"""

# ╔═╡ 997d7cb7-12b9-4650-b4f4-5da09009de6f
@inline function perturbate!(M::AbstractMatrix{<:Real}, σ::Real)
	@views M = M + rand(Normal(0.0, σ), size(M)...)
end

# ╔═╡ 727fc1d4-b581-4f94-a363-159412c24c8a
@inline perturbate(M::AbstractMatrix{<:Real}, σ::Real) = M + rand(Normal(0.0, σ), size(M)...)
...
@inline function cost(M_samples)
	corr_vec = mapreduce(ts_correlations, vcat, M_samples)
	hist = normalize(fit(Histogram, corr_vec, nbins=128), mode=:pdf)
...
function metropolis!(M_samples, β, n_iter)
	...
	for it ∈ 1:n_iter
		...
		if Δcost <= 0 || exp(-β*Δcost) > rand()
			M_samples = M_samples_new
			cost_prev = cost_new
		end
		costs[it+1] = cost_prev
	end
...
plot(x=mapreduce(ts_correlations, vcat,	M_samples_final),
	Geom.histogram(density=true))
