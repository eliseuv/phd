...
fig_mean = Figure(resolution=(800, 600))
axs_mean = [Axis(fig_mean[i, 1],
    limits=((nothing, nothing), (nothing, nothing)),
    xticks=[1, 2, 4, 6],
    yticks=[0.9, 0.95, 1])
            for i ∈ 1:3]
fig_var = Figure(resolution=(800, 600))
axs_var = [Axis(fig_var[i, 1],
    limits=((nothing, nothing), (nothing, nothing)),
    xticks=[1, 2, 4, 6],
    yticks=[0, 20, 40, 60])
           for i ∈ 1:3]
for (ax_mean, ax_var, D) ∈ zip(axs_mean, axs_var, D_vals_tcp)
    D_dict = eigvals_datafiles[D]
    T_c, transition_order, crit_temp_source = get_critical_temperature(df_temperatures, D)
    @assert transition_order == "tcp"
    transition_order_str = replace(transition_order, "first" => "First order", "second" => "Second order", "tcp" => "TCP")
    crit_temp_source_str = replace(string(crit_temp_source), "_" => " ")
    T_vec = sort(collect(keys(D_dict)))
    tau_vec = map(T_vec ./ T_c) do x
...
# xlabel = Label(fig_mean[4, 1], L"$\tau$")
ylabel = Label(fig_mean[1:3, 0], L"$\langle \lambda \rangle$", rotation=pi / 2)
save(joinpath(output_root, filename(global_prefix * "EigvalsMeans", "order" => "tcp"; ext="svg")), fig_mean)
# xlabel = Label(fig_var[4, 1], L"$\tau$")
# ylabel = Label(fig_var[1:3, 0], L"\langle \lambda^2 \rangle - \langle \lambda \rangle^2", rotation=pi / 2)
save(joinpath(output_root, filename(global_prefix * "EigvalsVars", "order" => "tcp"; ext="svg")), fig_var)
...
#     for (i, T) in enumerate(T_vec)
#         datafile = D_dict[T]
#         eigvals_max_spacings = map(maximum ∘ diff, eachcol(load_pickle(datafile.path)))
#         # mean_vec[i], var_vec[i] = hist_fluctuations(eigvals_spacings, 100)
#         mean_vec[i] = mean(eigvals_max_spacings)
#         var_vec[i] = var(eigvals_max_spacings)
#     end
